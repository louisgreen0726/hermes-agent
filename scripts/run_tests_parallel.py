...
Each test-file subprocess receives a private temporary ``HERMES_HOME`` before
pytest collection starts.  The per-test fixture replaces it again for test
execution, but this earlier boundary protects module-level imports that cache
profile paths during collection.
...
def _run_one_file_once(
    ...
    file_home_root = Path(tempfile.mkdtemp(prefix="hermes-test-file-"))
    try:
        file_home = file_home_root / "hermes_home"
        file_home.mkdir()
        child_env = os.environ.copy()
        child_env["HERMES_HOME"] = str(file_home)
        ...
        shutil.rmtree(file_home_root, ignore_errors=True)
