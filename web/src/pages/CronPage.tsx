...
import type { CronJob, CronDeliveryTarget, ModelOptionsResponse, ProfileInfo, SkillInfo, ToolsetInfo } from '@/lib/api'
import {
  buildCronJobPayload,
...
import { formatDateTime, formatMessage, formatNumber } from '@/lib/locale-format'
import type { Locale } from '@/i18n/types'

function formatTime(iso: string | null | undefined, locale: Locale): string {
  if (!iso) return '—'
...
function CronAdvancedFields({
  ...
  return (
    <details className="border border-border bg-background/30 p-3" open>
      <summary className="cursor-pointer text-xs font-medium uppercase tracking-normal text-muted-foreground">
...
function getRepeatDisplay(job: CronJob, foreverLabel: string, timesTemplate: string): string {
  ...
  return completed > 0 ? `${completed}/${repeat.times}` : formatMessage(timesTemplate, { count: repeat.times })
}
...
function profileLabel(profile: string, defaultLabel: string): string {
  return profile === 'default' ? defaultLabel : profile
}
...
export default function CronPage() {
  ...
  const resourceDialogOpen = createModalOpen || editJob !== null
  ...
  }, [resourceDialogOpen, resourceProfile])
  ...
  const handleCreate = async () => {
    ...
    if (!payload.schedule || (!payload.no_agent && !cronJobHasExecutionContent(payload))) {
      showToast(t.cron.promptScheduleRequired, 'error')
      return
    }
  ...
  const handlePauseResume = async (job: CronJob) => {
    try {
      ...
      if (isPaused) {
        ...
        showToast(`${t.cron.resume}: "${truncateText(getJobTitle(job, t.cron.cronJobFallback), 30)}"`, 'success')
      } else {
        await api.pauseCronJob(job.id, profile)
        showToast(`${t.cron.pause}: "${truncateText(getJobTitle(job, t.cron.cronJobFallback), 30)}"`, 'success')
      }
      loadJobs()
    } catch (e) {
  ...
  const handleTrigger = async (job: CronJob) => {
    try {
      ...
      showToast(`${t.cron.triggerNow}: "${truncateText(getJobTitle(job, t.cron.cronJobFallback), 30)}"`, 'success')
      loadJobs()
    } catch (e) {
      showToast(`${t.status.error}: ${e}`, 'error')
    }
    ...
    onDelete: useCallback(
      async (key: string) => {
        ...
        try {
          ...
          showToast(
            `${t.common.delete}: "${job ? truncateText(getJobTitle(job, t.cron.cronJobFallback), 30) : id}"`,
            'success'
          )
          loadJobs()
        } catch (e) {
      ...
      [jobs, loadJobs, showToast, t.common.delete, t.cron.cronJobFallback, t.status.error]
    )
  })
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Segmented
        ...
        options={[
          { value: 'jobs', label: t.cron.jobs },
          { value: 'blueprints', label: t.cron.blueprints }
        ]}
      />

      {view === 'blueprints' && (
        <AutomationBlueprints profile={selectedProfile === 'all' ? 'default' : selectedProfile} onCreated={loadJobs} />
      )}
      ...
      <DeleteConfirmDialog
        ...
        description={
          pendingJob
            ? `"${truncateText(getJobTitle(pendingJob, t.cron.cronJobFallback), 40)}" — ${t.cron.confirmDeleteMessage}`
        ...
        >
          ...
          >
            ...
            <header className="p-5 pb-3 border-b border-border">
              <h2 id="create-cron-title" className="font-mondwest text-display text-base tracking-normal">
                {t.cron.newJob}
              </h2>
            </header>
                ...
                <Select id="cron-profile" value={createProfile} onValueChange={v => setCreateProfile(v)}>
                  {profiles.map(profile => (
                    <SelectOption key={profile.name} value={profile.name}>
                      {profileLabel(profile.name, t.cron.defaultProfile)}
                    </SelectOption>
                  ))}
                </Select>
              </div>
      ...
      {editJob && (
        ...
        >
          ...
          >
            ...
            <header className="p-5 pb-3 border-b border-border">
              <h2 id="edit-cron-title" className="font-mondwest text-display text-base tracking-normal">
                {t.cron.editJob}
              </h2>
            </header>
            ...
            <div className="min-h-0 overflow-y-auto p-5 grid gap-4">
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  {saving ? t.common.saving : t.cron.saveChanges}
                </Button>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
              ...
              {t.cron.scheduledJobs} ({formatNumber(jobs.length, locale)})
            </H2>
          ...
          {jobs.length === 0 && (
            <Card>
              <CardContent className="py-8 text-center text-sm text-muted-foreground">{t.cron.noJobs}</CardContent>
            </Card>
          )}
          ...
          {jobs.map(job => {
            ...
            const title = getJobTitle(job, t.cron.cronJobFallback)
                ...
                <CardContent className="flex items-start gap-4 py-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge tone={STATUS_TONE[state] ?? 'secondary'}>{t.cron.states[state] ?? state}</Badge>
                      <Badge tone="outline">{profileLabel(profile, t.cron.defaultProfile)}</Badge>
                      {deliver && deliver !== 'local' && <Badge tone="outline">{deliver}</Badge>}
                      {Array.isArray(job.skills) && job.skills.length > 0 && (
                        <Badge tone="outline" title={job.skills.join(', ')}>
                          {job.skills.length === 1
                            ? job.skills[0]
                            : formatMessage(t.cron.skillCount, {
                                count: formatNumber(job.skills.length, locale)
                              })}
                        </Badge>
                      )}
                      ...
                      {toolsets.length > 0 && (
                        <Badge tone="outline" title={toolsets.join(', ')}>
                          {formatMessage(t.cron.toolsetCount, {
                            count: formatNumber(toolsets.length, locale)
                          })}
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span>
                        {t.cron.repeat}: {getRepeatDisplay(job, t.cron.repeatForever, t.cron.repeatTimes)}
                      </span>
                      <span>
                    ...
                    {job.last_delivery_error && (
                      <p className="text-xs text-destructive mt-1">
                        {t.cron.deliveryError}: {job.last_delivery_error}
                      </p>
                    )}
