...
import { formatNumber, formatRelativeSeconds } from "@/lib/locale-format";
...
export default function PairingPage() {
  ...
  const loadPairing = useCallback(() => {
    api
      ...
      .catch(() => showToast(t.pairing.failedToLoad, "error"))
      .finally(() => setLoading(false));
  }, [showToast, t.pairing.failedToLoad]);
  ...
  const handleApprove = async (user: PairingUser) => {
    ...
    try {
      ...
      showToast(
        t.pairing.approved.replace("{name}", getUserLabel(user)),
        "success",
      );
      loadPairing();
    } catch (e) {
      showToast(t.pairing.error.replace("{error}", String(e)), "error");
    } finally {
      setApproving(null);
    }
  };
  ...
  const handleClearPending = async () => {
    ...
    try {
      ...
      showToast(
        t.pairing.cleared.replace(
          "{count}",
          formatNumber(res.cleared, locale),
        ),
        "success",
      );
      loadPairing();
  ...
  const userRevoke = useConfirmDelete({
    onDelete: useCallback(
      ...
      [approved, loadPairing, showToast, t.pairing],
    ),
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <DeleteConfirmDialog
        ...
        description={
          pendingRevokeUser
            ? t.pairing.revokeNamedDescription.replace(
                "{name}",
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          ...
          {t.pairing.pendingRequests.replace(
            "{count}",
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 text-xs text-muted-foreground">
                    ...
                    {typeof user.age_minutes === "number" && (
                      <span>
                        {formatRelativeSeconds(-user.age_minutes * 60, locale)}
                      </span>
                    )}
                  </div>
                </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          ...
          {t.pairing.approvedUsers.replace(
            "{count}",
            formatNumber(approved.length, locale),
          )}
        </H2>
