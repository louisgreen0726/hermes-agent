...
export function PageHeaderProvider({
  ...
  return (
    <PageHeaderState key={pathname} pathname={pathname} pluginTabs={pluginTabs}>
      {children}
    </PageHeaderState>
  )
}

function PageHeaderState({
  children,
  pathname,
  pluginTabs
}: {
  ...
  const defaultTitle = useMemo(() => resolvePageTitle(pathname, t, pluginTabs), [pathname, t, pluginTabs])
      ...
      <div className="flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden">
        ...
        >
          <div
            className={cn(
              ...
              isChatRoute ? 'flex-row items-center' : 'flex-col justify-center sm:flex-row sm:items-center'
            )}
          >
            <div
              className={cn(
            ...
            >
              <h1
                className={cn(
                  'font-expanded min-w-0 text-sm font-bold tracking-normal text-foreground',
                  afterTitle && isEnvRoute
              ...
              {afterTitle ? (
                <div
                  className={cn(
                    ...
                    isEnvRoute ? 'w-full overflow-x-auto sm:flex-1 sm:overflow-x-auto' : 'shrink-0 overflow-visible'
                  )}
                >
                  {afterTitle}
                </div>
            ...
            {end ? (
              <div
                className={cn(
                  ...
                  isChatRoute ? 'w-auto shrink-0 justify-end' : 'w-full justify-start sm:justify-end'
                )}
              >
                {end}
              </div>
        ...
        <main
          className={cn(
            ...
            isChatRoute ? 'overflow-hidden' : 'overflow-y-auto overflow-x-hidden [scrollbar-gutter:stable]'
          )}
        >
          {children}
        </main>
