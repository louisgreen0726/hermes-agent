...
<p align="center">
  <a href="https://github.com/louisgreen0726/hermes-agent/releases"><img src="https://img.shields.io/badge/stable-Louis--0.20.0.4-2563eb" alt="稳定版本：Louis-0.20.0.4"></a>
...
| 维护状态 | 活跃，独立维护 |
| 生产分支 | `main` |
| 稳定基线 | [`Louis-0.20.0.4`](docs/releases/LOUIS_0.20.0.4.md) |
| Python 包版本 | `0.20.0+Louis.4` |
| 选择性上游基线 | Hermes Agent `0.20.0` / `2026.8.3` |
| 当前 `main` | `Louis-0.20.0.4` 稳定基线 |
| 与上游的关系 | 仅选择性评估和回迁 |
| 自动更新来源 | 仅 Louis `origin/main` |

多个具名自定义服务商可以共用同一个中转 URL，同时保持各自独立的模型列表、缓存、
API Key、API 模式、请求头、请求选项和凭据池；模型切换与会话恢复也会保留该身份。
...
- 对共用同一中转端点的具名服务商隔离分组、模型目录、缓存、凭据、传输、fallback
  和会话路由。
- 由候选版本自身控制的更新验证、回归测试门禁、回滚保护，以及对运行中的 Gateway
  和 Dashboard 进行安全重启。
